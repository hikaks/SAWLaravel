...
<head>
    ...
    <title>{{ $subject ?? 'Custom Notification' }}</title>
    <style>
        body {
...
<body>
    <div class="container">
        <div class="header">
            <h1>🔐 {{ $subject ?? 'SAW System Notification' }}</h1>
        </div>
        ...
        <div class="content">
            ...
            @if(isset($message))
                <div class="verification-info">
                    <h3>📧 {{ $subject ?? 'Email Verification Required' }}</h3>
                    <p>{{ $message }}</p>
                </div>
            @endif
        ...
        <div class="footer">
            <p><strong>{{ config('app.name', 'SAW Employee Evaluation System') }}</strong></p>
