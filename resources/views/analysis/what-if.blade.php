                    ...
                    <div class="mb-2">
                        ...
                        <div class="input-group">
                            <input type="range" class="form-range me-2" 
                                   ...
                                   oninput="syncWeightInputs({{ $criteria->id }}, 'range')">
                            <input type="number" class="form-control" 
                                   style="width: 80px;"
                                   min="1" max="100" 
                                   value="{{ $criteria->weight }}" 
                                   id="weightValue_{{ $criteria->id }}"
                                   oninput="syncWeightInputs({{ $criteria->id }}, 'number')">
            ...
            html = `
                <div class="mb-3">
                    ...
                    <select class="form-select" id="criteriaAction">
                        ...
                        <option value="modify">{{ __('Modify Criteria Weight') }}</option>
                    </select>
                </div>
...
function loadCriteriaOptions() {
    ...
    switch(action) {
        case 'remove':
            html = `
                <div class="mb-3">
                    <label class="form-label">{{ __('Select Criteria to Remove') }}</label>
                    <select class="form-select" id="criteriaToRemove" required>
                        <option value="">{{ __('Choose criteria...') }}</option>
                        @foreach($criterias as $criteria)
        ...
        case 'add':
            html = `
                <div class="mb-3">
                    <label class="form-label">{{ __('New Criteria Name') }}</label>
                    <input type="text" class="form-control" id="newCriteriaName" 
                           placeholder="{{ __('Enter criteria name') }}" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">{{ __('Weight') }} (%)</label>
                    <input type="number" class="form-control" id="newCriteriaWeight" 
                           min="1" max="100" value="10" required>
                    <div class="form-text">{{ __('Note: Total weights will be normalized automatically') }}</div>
                </div>
                <div class="mb-3">
                    <label class="form-label">{{ __('Criteria Type') }}</label>
                    <select class="form-select" id="newCriteriaType" required>
                        <option value="benefit">{{ __('Benefit (Higher is Better)') }}</option>
                        <option value="cost">{{ __('Cost (Lower is Better)') }}</option>
                    </select>
                </div>
            `;
            break;
        ...
        case 'modify':
            html = `
                <div class="mb-3">
                    <label class="form-label">{{ __('Select Criteria to Modify') }}</label>
                    <select class="form-select" id="criteriaToModify" required>
                        <option value="">{{ __('Choose criteria...') }}</option>
                        @foreach($criterias as $criteria)
                            <option value="{{ $criteria->id }}" data-current-weight="{{ $criteria->weight }}">{{ $criteria->name }} ({{ $criteria->weight }}%)</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3" id="modifyWeightSection" style="display: none;">
                    <label class="form-label">{{ __('New Weight') }} (%)</label>
                    <div class="input-group">
                        <input type="range" class="form-range me-2" 
                               min="1" max="100" 
                    ...
                    <div class="form-text">{{ __('Current weight: ') }}<span id="currentWeight"></span>%</div>
                </div>
            `;
            break;
    }
...
function getScenarioChanges(scenarioType) {
    switch(scenarioType) {
        case 'weight_changes':
            ...
            @foreach($criterias as $criteria)
                const weight{{ $criteria->id }} = parseInt($('#weightValue_{{ $criteria->id }}').val());
                if (!isNaN(weight{{ $criteria->id }}) && weight{{ $criteria->id }} > 0) {
                    changes[{{ $criteria->id }}] = weight{{ $criteria->id }};
