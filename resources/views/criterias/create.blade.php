...
@section('title', 'Add Criteria - SAW Employee Evaluation')
@section('page-title', 'Add Evaluation Criteria')
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">
                    ...
                    Add Evaluation Criteria Form
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('criterias.store') }}" method="POST" id="criteriaForm" novalidate>
                    @csrf
                            ...
                            <div class="col-md-6">
                                <div class="form-check form-check-lg">
                                    <input class="form-check-input"
                                           ...
                                           id="typeBenefit"
                                           value="benefit"
                                           {{ old('type', 'benefit') == 'benefit' ? 'checked' : '' }}
                                           required>
                    ...
                    <div class="card bg-light mb-4">
                        <div class="card-body">
                            ...
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    <div class="mb-2">
                                        <span>Existing weight: <strong>{{ $totalWeight }}%</strong></span>
                                    </div>
                                    <div class="mb-2">
                                        <span>New criteria weight: <strong id="newWeightDisplay">0%</strong></span>
                                    </div>
                                    <div class="mb-2">
                                        <span>Total after adding: <strong id="totalAfterDisplay">{{ $totalWeight }}%</strong></span>
                                    </div>
                                    <div class="progress" style="height: 20px;">
                                        <div class="progress-bar bg-secondary"
                                             style="width: {{ $totalWeight }}%"
                                             title="Existing Weight: {{ $totalWeight }}%">
                                        </div>
...
@push('scripts')
<script>
const maxWeight = {{ $remainingWeight }};
...
function updateWeightVisualization() {
    ...
    // Update progress bar
    const maxBarWidth = Math.min(newWeight, 100 - currentTotal);
    $('#newWeightBar').css('width', (maxBarWidth) + '%')
                      .attr('title', `Bobot Baru: ${newWeight}%`);
    ...
    $('#criteriaForm input[required], #criteriaForm input[type="radio"][required]').each(function() {
        if (this.type === 'radio') {
            if (!$(`input[name="${this.name}"]:checked`).length) {
                isValid = false;
                $(`#${this.name}_error`).text('Field ini wajib dipilih');
            }
        } else {
